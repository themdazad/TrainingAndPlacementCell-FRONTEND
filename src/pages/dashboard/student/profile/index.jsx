          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-2 mt-3">
                <Chip color="primary" variant="flat">{profile?.branch || 'Branch'}</Chip>
                <Chip color="secondary" variant="flat">Batch {profile?.batch || 'N/A'}</Chip>
          ...
          <CardBody className="p-6 space-y-6">
            <ProfileSection title="Academic Information">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <InfoRow label="10th Percentage" value={profile?.academicInfo?.tenthPercentage ? `${profile.academicInfo.tenthPercentage}%` : '-'} />
                <InfoRow label="12th Percentage" value={profile?.academicInfo?.twelfthPercentage ? `${profile.academicInfo.twelfthPercentage}%` : '-'} />
              </div>
            </ProfileSection>
            ...
            <ProfileSection title="Active Backlogs">
              <div className="flex items-center gap-4">
                <Chip color={profile?.academicInfo?.activeBacklogs > 0 ? 'danger' : 'success'} variant="flat" size="lg">
              ...
              <div className="flex flex-wrap gap-2">
                {profile?.skills?.length > 0 ? (
                  profile.skills.map((skill) => (
                    <Chip key={skill} variant="bordered">{skill}</Chip>
                  ))
                ) : (
                ...
                {profile?.documents?.resume && (
                  <Card className="bg-default-50">
                    <CardBody className="flex flex-row items-center gap-4">
                      ...
                      <Button size="sm" variant="flat" as="a" href={profile.documents.resume} target="_blank">
                        View
                      </Button>
                    </CardBody>
                  </Card>
                ...
                {!profile?.documents?.resume && (
                  <Card className="bg-default-50 border-2 border-dashed border-default-200">
                    <CardBody className="flex flex-col items-center justify-center py-8">
                      <svg className="w-12 h-12 text-default-300 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
